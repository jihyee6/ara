...
<%@ include file="web_cert_conf.jsp"%>
<%
    /* ============================================================================== */
    /* =   인증데이터 수신 및 복호화 페이지                                         = */
    /* = -------------------------------------------------------------------------- = */
...
<%
    ...
    // 결과 처리
        if( res_cd.equals( "0000" ) )
        {
            ...
            user_name = cc.getKeyValue("user_name");
            phone_no = cc.getKeyValue("phone_no");
            gender = cc.getKeyValue("sex_code");
            birthYear = cc.getKeyValue("birth_day");
        }
    ...
    <body>
         <script>
	        $(function() {
	        	 $.ajax({
					...
					success : function(result) {
						...
						opener.location.href = "joinInput";
						window.close();
						return false;
					},
					error : function() {
	        ...
	        <%--  $().ready(function() {
	         	...
	         	opener.getUserInfo('<%= user_name%>', '<%= phone_no%>', '<%= gender%>', '<%= birthYear%>')
